...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.startPosition = self.startingPosition
        self.startState = (self.startPosition, (False, False, False, False))
    def getStartState(self):
        """
    ...
    def getResult(self, state, action):
        ...
        return (new_position, tuple(visited_corners))
...
def cornersHeuristic(state, problem):
    ...
    #if all corners were visited, then it will return 0
    if not unvisited:
        return 0

    #calculating the distance from current position to the rest of the unvisited corners left
    distances = []
    for corner in unvisited:
        distances.append(util.manhattanDistance(position, corner))


    # where we are returning the heuristic, which is the max distance to the farthest unvisited corner
...
def foodHeuristic(state, problem):
    ...
    #if there is no food left, then the heuristic value has to be zero, so return zero
    if not foodList:
        return 0

    #returns the max manhattan distance from the current position to any food
